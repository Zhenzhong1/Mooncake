...
static std::vector<TopologyEntry> discoverHpuTopology(
    const std::vector<InfinibandDevice> &all_hca) {
    std::vector<TopologyEntry> topology;

    uint32_t device_count = 0;
    if (synDeviceGetCount(&device_count) != synSuccess) {
        device_count = 0;
    }
    ...
    for (uint32_t i = 0; i < device_count; i++) {
        ...
        if (synDeviceAcquireByDeviceType(&device_id, synDeviceGaudi) != synSuccess &&
            synDeviceAcquireByDeviceType(&device_id, synDeviceGaudi2) != synSuccess &&
            synDeviceAcquireByDeviceType(&device_id, synDeviceGaudi3) != synSuccess) {
            continue;
        }

        char pci_bus_id[20];
        if (synDeviceGetPCIBusId(pci_bus_id, sizeof(pci_bus_id), device_id) != synSuccess) {
            synDeviceRelease(device_id);
            continue;
        }
        ...
        topology.push_back(
            TopologyEntry{.name = "hpu:" + std::to_string(device_id),
                          .preferred_hca = std::move(preferred_hca),
                          .avail_hca = std::move(avail_hca)});
...
#ifdef USE_HPU
    for (auto &ent : discoverHpuTopology(all_hca)) {
        matrix_[ent.name] = ent;
    }
#endif
